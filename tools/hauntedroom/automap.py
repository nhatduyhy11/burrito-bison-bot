...
AUTOMAP_TEMPLATE_THRESHOLD = 0.9
AUTOMAP_POLL_MS = 400
...
def region_has_red(
    ...
    blue, green, red = np.moveaxis(image[y1:y2, x1:x2], -1, 0)
    red_pixels = (
        (red >= 140)
        & (red.astype(np.int16) - green.astype(np.int16) >= 50)
        & (red.astype(np.int16) - blue.astype(np.int16) >= 50)
    )
